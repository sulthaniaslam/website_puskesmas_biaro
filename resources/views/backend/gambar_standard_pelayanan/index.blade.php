...
@section('title_admin', 'Gambar Standard Pelayanan')
                                ...
                                <tr class="text-center">
                                    ...
                                    <td>
                                        <img src='{{ asset("images/standard_pelayanan/$index->gambar_standard_pelayanan") }}'
                                            width="100" alt="Gambar Standard Pelayanan">
                                    </td>
                                    <td>
                                        <span class="badge badge bg-success mb-1" data-toggle="tooltip"
                                            data-placement="top" title="Edit">
                                            <a href="{{ route('edit_pelayanan_gambar', Crypt::encrypt($index->id_gambar_standard_pelayanan)) }}"
                                        ...
                                        <form method="POST" id="formHeroDelete">
                                            ...
                                            @csrf
                                            <button type="button"
                                                onclick="hapusGambarStandardPelayanan('{{ $index->id_gambar_standard_pelayanan }}')"
...
<script>
    function tambahData() {
      var id = "{{ $id }}";
      var url = "{{ url('/admin-puskesmas/create_pelayanan_gambar/') }}/" + id;        
        ...
        function hapusGambarStandardPelayanan(id) {
            var id_hapus = id;
            Swal.fire({
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: "{{ route('hapus_pelayanan_gambar') }}",
                            data: {
                                "_token": "{{ csrf_token() }}",
                                "id_hapus": id_hapus
                            },
